...
<div class="max-w-2xl mx-auto space-y-6">
  ...
  <section class="space-y-3">
    ...
    <label class="flex items-center justify-between p-3 rounded-lg border border-[var(--color-border)]">
      <div>
        <p class="text-sm font-medium">{m.settings_clipboard_monitoring_label()}</p>
        <p class="text-xs text-[var(--color-text-secondary)]">{m.settings_clipboard_monitoring_desc()}</p>
      </div>
      <input type="checkbox" checked={settingsStore.settings.clipboard_monitoring} onchange={() => toggle("clipboard_monitoring")}
        class="w-4 h-4 accent-[var(--color-primary)]" />
    </label>
